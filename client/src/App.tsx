...
function VoicePlanner({ trip, onTrip }: { trip: Trip; onTrip: (trip: Trip, note: string) => void }) {
  ...
  const createPlan = async () => {
    ...
    try {
      ...
      onTrip(response.trip, response.itinerarySource === 'google-places' ? 'Real attractions sourced from Google Places and mapped into your trip.' : `Curated fallback route: ${response.placesDiagnostic ?? 'Google Places did not return enough attractions.'}`);
...
function BookingCheckout({ trip, onTrip }: { trip: Trip; onTrip: (trip: Trip, note: string) => void }) {
  ...
  const selectFlight = async (id: string) => {
    setBusy(true);
    try { const response = await api.selectFlight(id); setConfirmed(false); setOrder(null); onTrip(response.trip, 'Flight choice updated. Confirm your booking choices before checkout.'); }
  ...
  const selectHotel = async (id: string) => {
    setBusy(true);
    try { const response = await api.selectHotel(id); setConfirmed(false); setOrder(null); onTrip(response.trip, 'Hotel choice updated. Confirm your booking choices before checkout.'); }
...
function PreferenceDecision({ trip, onTrip }: { trip: Trip; onTrip: (trip: Trip, note: string) => void }) {
  ...
  const approve = async () => {
    setSaving(true);
    try {
      const response = await api.approvePreferences(scores);
...
function App() {
  ...
  const content = page === 'home' ? <><TripOverview trip={trip} setPage={setPage} activeDay={activeDay} setActiveDay={setActiveDay} onReceipt={() => void scanReceipt()} /><div className="mt-6"><TravelerProfiles trip={trip} /></div></> : page === 'planner' ? <><VoicePlanner trip={trip} onTrip={onTrip} /><PreferenceDecision trip={trip} onTrip={onTrip} /></> : page === 'map' ? <JourneyMap trip={trip} activeDay={activeDay} setActiveDay={setActiveDay} /> : page === 'operations' ? <OperationsCenter trip={trip} onTrip={onTrip} /> : <BookingCheckout trip={trip} onTrip={onTrip} />;
