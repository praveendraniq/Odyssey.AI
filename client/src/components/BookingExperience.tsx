...
const airportCode = (place: string) => {
  ...
  const cityCodes: Record<string, string> = {
    'SAN FRANCISCO': 'SFO', 'LOS ANGELES': 'LAX', DALLAS: 'DFW', 'SAN DIEGO': 'SAN',
    HAWAII: 'HON', HONOLULU: 'HNL', MAUI: 'OGG', TOKYO: 'NRT', JAPAN: 'NRT',
    PARIS: 'CDG', LONDON: 'LHR', DELHI: 'DEL', MUMBAI: 'BOM', CHENNAI: 'MAA',
  };
...
export function BookingExperience({ trip, onTrip, autoSearchRun = 0 }: { trip: Trip; onTrip: (trip: Trip, note: string) => void; autoSearchRun?: number }) {
  ...
  return <div className="space-y-6 pb-32">
    ...
    {stage === 'review' ? <>
      ...
      <section className="hidden rounded-[28px] border border-sky-200 bg-sky-50 p-5" aria-hidden="true">
      ...
      <section className="flex flex-col justify-between gap-4 rounded-[28px] bg-[#eff6f1] p-6 sm:flex-row sm:items-center"><div><p className="eyebrow text-moss">Admin confirmation</p><h2 className="mt-1 text-2xl font-bold">{selectedLiveBundle ? `Confirm ${selectedLiveBundle.label}` : selectedBundle ? `Confirm ${selectedBundle.label} demo bundle` : 'Choose a trip bundle'}</h2><p className="mt-2 text-sm text-stone-600">{selectedLiveBundle ? `${selectedLiveBundle.flight.title} plus ${selectedLiveBundle.hotel.title} · ${money(bookingTotal)} group total.` : selectedBundle ? `${selectedBundle.flight.airline} ${selectedBundle.flight.code} plus ${selectedBundle.hotel.name} · ${money(bookingTotal)} group total. Sabre live inventory is unavailable; this curated bundle can still be paid via PayPal.` : 'Select a live Sabre option or curated demo bundle before continuing to payment.'}</p></div><button disabled={!selectedLiveBundle && !selectedBundle} onClick={() => { setConfirmed(true); setStage('payment'); }} className="rounded-xl bg-moss px-5 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><CheckCircle2 className="mr-2 inline" size={16} />Confirm & open admin payment</button></section>
    </> : <>
      ...
      {false && order?.status === 'COMPLETED' && <CertTestBookingCard
...
function BookingAgentVoiceControls({ trip, selectedFlight, selectedHotel, bookingTotal, onSelectBundle, onConfirm, onCollectPayment }: BookingVoiceContext) {
  ...
  useEffect(() => {
    ...
    void sendAction('journeyos_context', {
      ...
      selectedHotel,
      bookingTotal,
      instruction: 'You are the separate Odyssey Booking Agent. Identify yourself as the Booking Agent in your first sentence. You handle only this Booking & Payment page: explain the selected flight and hotel, compare bundles when asked, and prepare payment review. Do not speak as the Travel Mediator, restart trip planning, collect friend preferences, negotiate, or manage itinerary changes. Never claim supplier booking, airline ticketing, hotel confirmation, or payment capture without explicit app confirmation.',
    });
  ...
  return <section className="rounded-[28px] border border-sky-200 bg-sky-50 p-5 sm:flex sm:items-center sm:justify-between sm:gap-6">
    <div className="min-w-0 flex-1"><p className="eyebrow text-sky-800">Separate Vocal Bridge agent</p><h2 className="mt-1 text-xl font-bold text-ink">Talk to the Booking Agent</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-stone-600">It knows this page&apos;s selected flight, hotel, travel dates, and total. It can explain options and prepare the admin for PayPal Sandbox, but never claims a Sabre supplier booking.</p>{voice.error && <p className="mt-2 text-xs font-semibold text-coral">{voice.error.message}</p>}</div>
    <button type="button" onClick={() => setShowTranscript((current) => !current)} className="mt-4 rounded-lg px-2 py-1 text-[10px] font-bold text-sky-800 hover:bg-white sm:mt-0">Transcript</button>
    ...
    {showTranscript && <div className="fixed bottom-24 right-5 z-[70] w-[min(420px,calc(100vw-2.5rem))] overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-2xl"><div className="flex items-center justify-between bg-sky-900 px-4 py-3 text-white"><div><p className="text-[10px] font-bold uppercase tracking-wider text-sky-200">Booking Agent transcript</p><p className="mt-1 text-sm font-bold">Your conversation</p></div><button onClick={() => setShowTranscript(false)} className="text-xs font-bold text-white/70 hover:text-white">Close</button></div><div className="max-h-64 space-y-2 overflow-y-auto p-4">{transcript.length ? transcript.slice(-12).map((entry, index) => <div key={`${entry.role}-${index}`} className={`rounded-xl px-3 py-2 text-xs leading-5 ${entry.role === 'user' ? 'ml-8 bg-sky-50 text-ink' : 'mr-8 bg-stone-100 text-stone-700'}`}><b>{entry.role === 'user' ? 'You' : 'Booking Agent'}:</b> {entry.text}</div>) : <p className="text-sm text-stone-500">Start talking to the Booking Agent and the transcript will appear here.</p>}</div></div>}
  </section>;
}
